    ...
    for (let i in post.blocks) {
      switch (post.blocks[i].type) {
        case 'image':
          ...
          console.log(post.blocks[i].image)
          if (post.blocks[i].image.type === 'external') {
            img.src = `/api/imageproxy?url=${encodeURIComponent(img.src)}`
          } else if (post.blocks[i].image.type === 'file') {
            img.src = `/api/imageproxy?id=${post.blocks[i].id}`
          }
          post.blocks[i] = {
            type: 'image',
            image: {
          ...
          if (post.image === config.defaultPostImage) {
            post.image = `${config.url}${img.src}`
          }
          break
        case 'bookmark':
          const metadata = await fetchMeta(post.blocks[i].bookmark.url)
          post.blocks[i] = {
            type: 'bookmark',
            bookmark: metadata,
          }
